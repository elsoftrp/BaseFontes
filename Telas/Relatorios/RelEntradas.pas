...
begin
  ...
  begin
    TLib.Mensagem('Aviso','Fornecedor não encontrado.','!');
...
begin
  ...
  begin
    try
      ...
      if CodFor.Text <> '' then
        SQLRelEntradasResumido.ParamByName('CodFor').AsInteger := StrToInt(CodFor.Text);
...
begin
  ...
  begin
    try
      vsSql := 'select e.*, p.despro, i.codpro, i.qde, i.valor, i.total, i.numerolote, '+

...
uses Funcoes, DataModulo, Senha,
  ...
  ConsultaMovEstoque, RelProdutos, RelEntradas, RelVendas, CadCartao;
...
begin
  ...
  //-------------------------------------------------------
  if Prog = UpperCase('CartaoCredito') then
  begin
    TLib.AbreForm(TfrmCadCartao, frmCadCartao);
    exit;
  end;
  //-------------------------------------------------------
  ...
  begin
    ...
    begin
      SQLClientes.Open;
      try
        frxReport1.LoadFromFile(TLoadFileIni.GetInstance.PathRelat+'\frClientes.fr3');
...
begin
  ...
  //
  y:=y+1;
  prog[y,1] := 'CartaoCredito';
  prog[y,2] := 'Cadastro de Cartão';
  prog[y,3] := '10 &Cadastro';
  prog[y,4] := '99 &Manutenção';
  prog[y,5] := '01 &Cartão';
  //
  y:=y+1;
  prog[y,1] := 'Produtos';
